...
def show_comprehensive_analysis_page_parallel():
    ...
    if can_generate:
        ...
        if analysis_status == "processing":
            ...
            # Trigger actual processing - start both analyses simultaneously
            if 'analysis_processing_started' not in st.session_state:
                ...
                # Start both analyses in parallel (simulated with sequential calls but both triggered)
                ...
                if st.session_state.get('quick_analysis_completed') and st.session_state.get('comprehensive_analysis_completed'):
                    # Combine sections
                    all_sections = {}
                ...
                elif st.session_state.get('quick_analysis_completed'):
                    # Show quick results while comprehensive is still running
                    st.session_state.analysis_status = "quick_completed"
                elif st.session_state.get('comprehensive_analysis_completed'):
                    # Show comprehensive results while quick is still running
                    st.session_state.analysis_status = "comprehensive_completed"
                else:
                    # Both failed
                    st.session_state.analysis_status = "error"
                    st.session_state.analysis_error = "Both analyses failed"
        ...
        elif analysis_status == "comprehensive_completed":
            # Show comprehensive analysis results while quick is running
            st.success("✅ Comprehensive analysis completed! Quick analysis in progress...")
            st.info("⏱️ **AI processing may take up to 2 minutes** - please be patient!")
            
            # Show comprehensive analysis results
            if 'comprehensive_analysis_sections_parallel' in st.session_state:
                sections = st.session_state['comprehensive_analysis_sections_parallel']
                ...
                st.info("🔄 Quick analysis (roast + recommendations) still in progress...")
            
            # Auto-refresh to check for quick completion
            import time
            current_time = time.time()
            if 'last_refresh' not in st.session_state:
                st.session_state.last_refresh = current_time
